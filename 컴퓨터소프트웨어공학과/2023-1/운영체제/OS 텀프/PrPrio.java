...
public class PrPrio {
    ...
    void startSchedule(Process p, int index) {
        ...
        while (cpuCount != p.timeRemain) {
            ...
            if (isNew) {
                isNew = false;
                if (p.priority > rq.get(rq.size() - 1).priority) {
                    break;
                }
            }
        }
        usages = false;
        if (cpuCount == p.timeRemain) {
            ...
            p.timeRemain = p.timeRemain - cpuCount;
            rq.add(p);
        }
